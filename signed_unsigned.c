...
** Last update Mon Nov 16 19:47:50 2015 Baptiste veyssiere
*/
...
void    case_short(va_list ap, const char *list, int *i, int *length)
{
  if (list[*i + 1] == 'i' || list[*i + 1] == 'd')
    case_signed_short(ap, list, i, length);
  if (list[*i + 1] == 'u')
    case_unsigned_short(ap, list, i, length);
  *i += 1;
}
...
{
  if (list[*i + 1] =='i' || list[*i + 1] == 'd')
    case_signed_long(ap, list, i, length);
  if (list[*i + 1] =='u')
    case_unsigned_long(ap, list, i, length);
  *i += 1;
}
